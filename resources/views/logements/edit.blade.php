<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto pt-28 pb-12 px-4 sm:px-6">
        ...
        <!-- Header -->
        <div class="mb-10">
            <h1 class="text-3xl font-black text-gray-900 tracking-tight">Modifier le Logement</h1>
            <p class="text-gray-500 mt-1">Mettez à jour les informations de votre annonce : <strong>{{ $logement->titre }}</strong></p>
        </div>
        ...
        <form action="{{ route('logements.update', $logement->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                                  class="w-full bg-gray-50 border border-gray-300 p-4 text-gray-800 focus:ring-[#445EF2]/20 placeholder-gray-300">{{ old('description', $logement->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" class="w-full bg-gray-50 border border-gray-300 p-4 text-gray-800 focus:ring-[#445EF2]/20">
                            ...
                            <option value="chambre" {{ old('type', $logement->type) == 'chambre' ? 'selected' : '' }}>Chambre</option>
                            <option value="studio" {{ old('type', $logement->type) == 'studio' ? 'selected' : '' }}>Studio</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-100 p-8 shadow-sm">
                ...
                <!-- Affichage des images actuelles (Optionnel mais recommandé) -->
                <div class="grid grid-cols-4 gap-4 mb-6">
                    @foreach($logement->images as $image)
                        <div class="relative h-24 bg-gray-100 border border-gray-200">
                ...
                <div class="relative border-2 border-dashed border-gray-200 p-10 text-center hover:border-[#445EF2] group">
                    ...
                    <div class="space-y-2">
                        <i class="fas fa-plus-circle text-4xl text-gray-300 group-hover:text-[#445EF2] transition-colors"></i>
                        <p class="text-gray-500 font-medium">Ajouter de nouvelles photos</p>
                        <p class="text-xs text-gray-600 uppercase font-black">PNG, JPG (Les anciennes seront conservées)</p>
                    </div>
                </div>
            </div>
